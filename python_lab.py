...
S_intersect_T = {S for S in {1,2,3,4} for T in {3,4,5,6} if S==T}
...
LofL_sum = sum(sum([[.25, .75, .1], [-1, 0], [4, 4, 4, 4]],[]))
...
odd_num_list_range = {n for n in range(100) if n%2!=0}
...
list_sum_zip = [sum(x) for x in list(zip([10,25,40],[1,15,20]))]
...
value_list = [x['James'] for x in {'James':'Sean', 'director':'Terence'}, {'James':'Roger',
...
value_list_modified_1 = [x['Bilbo'] if'Bilbo' in x else 'NOT PRESENT' for x in {'Bilbo':'Ian','Frodo':'Elijah'},
{'Bilbo':'Martin','Thorin':'Richard'}]  # <-- Use the same expression here
k = 'Frodo'
value_list_modified_2 = [x['Frodo'] if 'Frodo' in x else 'NOT PRESENT' for x in {'Bilbo':'Ian','Frodo':'Elijah'},
{'Bilbo':'Martin','Thorin':'Richard'}]  # <-- as you do here
...
D = {'red','white','blue'}
# Replace {...} with a one-line dictionary comprehension
identity_dict = { x for x in {'red','white','blue'}}
...
def nextInts(L): return [L[i]+1]



## 27: (Task 27) Procedure cubes
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def cubes(L): return [pow(L[i],3)] 
...
# Complete the procedure definition by replacing [ ... ] with a one-line list comprehension
def dict2list(dct, keylist): return [keylist:dct]
...
def list2dict(L, keylist): return {L:keylist}
